...
import ScrollGenderIntervals from './scroll_gender_intervals.imba'

Chart.register(...registerables);

export default tag AgeSegmentIntervals
	...
	def get_data
		gr_data = []
		sum_intervals = 0
		for obs in element.metrics[0].last
			if Array.isArray(obs.observed)
				gr_data.push(obs.observed[hour_set] or 0)
				sum_intervals += obs.observed[hour_set] or 0
				hour_chart = true 
			else
				gr_data.push(obs.observed or 0)
				sum_intervals += obs.observed or 0

		if not element.current_segment
			element.current_segment = ""
		if element.metrics[0].sum and element.metrics[0].sum[element.current_segment]
			sum_intervals = element.metrics[0].sum[element.current_segment].observed
	...
	def get_config_data
		const data = get_data()
		return
			labels: element.element_def.segment_keys[1]
			datasets: [
				axis: "y"
				backgroundColor: backgroundColors.slice(0, data.length - 1)
				borderColor: borderColors.slice(0, data.length - 1)
				borderWidth: 1
				data: data
			]
		...
		if typeof element.max_hour == "number"
			...
			if element.text.includes("\{hour\}")
				...
				header = <span> 
					...
					hour_set 
					els[1]
					" / " + (element.instancedata.segment or "Alle") + " / : Total " + sum_intervals
			else 	
				header = <span> element.text + " / " + (element.current_segment or "Alle") + " /  : Total " + sum_intervals
						...
						<a.right-icons>
							...
							if postarrow
								<a.button-item-icon @click=incr_hour>
									postarrow
							<ScrollGenderIntervals element=element>
					<p>
