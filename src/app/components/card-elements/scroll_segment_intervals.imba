...
export default tag ScrollSegmentIntervals
	...
	def new_instance_data(current, delta)
		def set_instance_data()
			element.set_instance_data("segment", segment_keys[0][current[0]+delta[0]] + segment_keys[1][current[1]+delta[1]])
	...
	def getindexes()
		let gender = ""
		let age = ""
		if not element.instancedata.segment
			return 0, 0

		gender = Math.max(segment_keys[0].indexOf(element.instancedata.segment.substring(0,1)), 0)
		if gender and element.instancedata.segment.length > 1
			age = segment_keys[1].indexOf(element.instancedata.segment.substring(1))
		elif gender
			age = 0
		else
			age = segment_keys[1].indexOf(element.instancedata.segment)
	...
	def render
		if not segment_keys
			segment_keys = element.element_def.segment_keys or state.user.userdata.globals.segment_keys
		current_index = getindexes()
		...
		if current_index[0] > 0
			clicks[0] = new_instance_data(current_index, [-1, 0]) 
			titles[0] = segment_keys[0][current_index[0] - 1] or "All"
			colors[0] = true
		if current_index[1] < segment_keys[1].length - 1
			clicks[1] = new_instance_data(current_index, [0, 1]) 
			titles[1] = segment_keys[1][current_index[0] + 1] or "All"
			colors[1] = true
		if current_index[0] < segment_keys[0].length - 1
			clicks[2] = new_instance_data(current_index, [1, 0]) 
			titles[2] = segment_keys[0][current_index[1] + 1] or "All"
			colors[2] = true
		if current_index[1] > 0
			clicks[3] = new_instance_data(current_index, [0, -1]) 
			titles[3] = segment_keys[1][current_index[1] + 1] or "All"
			colors[3] = true
		...
		<self>
			<div.ic-scroll-button>
				...
				<span.symbol[box-sizing:border-box] .t1=ok .l1=ok .oc=ok @click=element.flip_order title=title="order">
					if order == 'C'
						<a[box-sizing:border-box]> accum
					else	
						<a[box-sizing:border-box]> normal